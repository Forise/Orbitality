...
public class UIWindow : StateMachine
{
    ...
    protected virtual void ActiveStateInitHandler()
    {
        ...
        EventManager_Input.Subscribe(EventManager_Input.BACK_BUTTON_PRESSED, Handler_BackButton);
    ...
    protected virtual void ActiveStateCloseHandler()
    {
        ...
        EventManager_Input.Unsubscribe(EventManager_Input.BACK_BUTTON_PRESSED, Handler_BackButton);
    }
    ...
    {
        if (closeByBackButton && UIController.Instance.IsTopWindow(ID))
            CloseThisWindow();
    }
    ...
    protected virtual void Handler_OpenWindow(object sender, GameEventArgs e)
    {
        if (e.str != id)
            return;
        if (sender.GetType() != typeof(UIController))
            Debugger.LogWarning($"Sender is not UIController! Sender is {sender.ToString()}", this);
        SetState((int)WindowStates.OpenAnimation);
    }
